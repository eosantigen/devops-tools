...
variable "virtual_network_name" {
  ...
  validation {
    ...
    error_message = "Each Virtual Network must start with vnet-, contain the service-name as eg -aks, and end with -<ENV_NAME>. Case insensitive."
  }
}
...
variable "virtual_network_resource_group_name" {
  ...
  validation {
    ...
    error_message = "Each Resource Group must start with rg-, include the desired Resource Type i.e. -aks and end with -<ENV_NAME>. Case insensitive."
  }
}
...
variable "subnets" {
  ...
  # validation {
  #   condition = can([for s in var.subnets : regex("(?i:subnet-\\w))", s)])
  #   error_message = "Each subnet name must start with subnet-, contain the service-name as eg -aks, and end with -<ENV_NAME>. Case insensitive."
  # }
}
...
# NOTE: This is left here as useful bit of info.
