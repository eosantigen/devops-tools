...
send Slack notifications to the relevant channel upon any successful deployment (except for the Secrets).
...
# Per Client/Environment Configuration - Wrapper shell scripts

Some Bash shell wrapper scripts have been implemented as a custom solution to manage different states and environments (also called “workspaces”), as well as attain to a more easy and secure to implement, execute & manage solution. This is because these scripts only prepare the plan based on targeted resources named as “deployments”, and do not execute the final apply command implicitly.
...
3. Find values for the <client_name>.tfvars and add them accordingly:
    ...
    - Depending on the deployment you want, create the new workspace (an isolated state space for each client). terraform -chdir=../azure/deployments/<deployment_name> workspace new <client_name>
    
4. `./terraform.sh -c <client_name> -d <deployment_name> [ -t <'name.reference_name'> ]`

If 3 is ok, a plan has been prepared. Triple-check all shown in the output is correct and then : `terraform -chdir=../azure/deployments/<deployment_name>/  apply "/tmp/plan-<client_name>-<deployment_name>"`  or if  -t is  specified: `“/tmp/plan-target-<client_name>-<deployment_name>"`
...
**Create Databricks PaS instance**
```
...
**Create Databricks instance Secrets Scopes**
```
...
**Create Databricks instance Secrets**
```
...
`terraform -chdir=../azure/deployments/<deployment_name> workspace select <client_name>` _(there is an auto-switch within the wrapper script but always do it here, too)_

Also, it’s not a bad idea to inspect the current state of this workspace (=> "client-env-X") for this deployment type (=> databricks_secret):
...
Then, run `./terraform.sh -c client-env-X -d databricks_secret`
...
Example: you have a client 'client-env-X' with a deployment 'databricks' within some Azure Subscription with some <SUBSCRIPTION_ID> - then, just replace the full ID accordingly (can be found from the Portal->Properties) - and execute:
...
- Although this project seems complex, it is actually very easy! Other than some caution details, even Koutsi Maria can execute - and even extend - it!  
